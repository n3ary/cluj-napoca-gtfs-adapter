...
This repo overrides the default `bump-skip-paths` to also include `docs/` (in addition to the default `.github/`, `.gitignore`, `LICENSE`) because the daily workflow's `paths-ignore` already excludes docs-only changes.