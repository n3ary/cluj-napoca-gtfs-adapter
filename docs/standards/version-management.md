...
Cross-ref: [../../DEVELOPMENT.md](../../DEVELOPMENT.md) and the [README's "Contributing" section](../../README.md#contributing) for the implementation walkthrough (this standard is the rule; those docs document how it's wired up).
...
- The daily workflow (which runs on `push: branches: [main]`) needs the version already incremented when it runs, so the published `cluj-napoca.gtfs.zip` artifact reflects the right number.
...
- **Don't bump on merge.** Adds a race between the bump commit and the daily workflow; can produce artifacts that report the wrong version.
...
- **Don't bump on a schedule.** Schedule-based bumps cause version drift between the source and the published artifact; the bump should always accompany a code change.
...
- The workflow YAML is portable; copy it from [neary's reference](https://github.com/ciotlosm/neary/blob/main/.github/workflows/pr-validation.yml) and adjust the validation commands to match this repo's tooling (`npm run validate`, `npm test`, `npm run reconcile:dry`).